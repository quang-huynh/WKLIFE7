...
model_select_GH <- function(mlen, ss = rep(1, length(mlen)), K, Linf, Lc) {
  ...
  stpar <- c(rep(0.5, 2), length(mlen)/2)
  res1 <- try(optim(stpar, GH_LL, method = "BFGS", 
  ...
  stpar <- c(rep(0.5, 3), c(1, 2) * length(mlen)/3)
  res2 <- try(optim(stpar, GH_LL, method = "BFGS", 
                    Lbar = mlen, ss = ss, nbreaks = 2, 
                    K = K, Linf = Linf, Lc = Lc, control = list(maxit = 1e+06), 
                    hessian = FALSE), silent = TRUE)
  if(inherits(res2, 'try-error')) aic2 <- NA
  else aic2 <- 2 * (res1$value + 6)
  
  stpar <- c(rep(0.5, 4), c(1, 2, 3) * length(mlen)/4)
  res3 <- try(optim(stpar, GH_LL, method = "BFGS",
  ...
  if(all(is.na(testvec))) Z.ans <- NA
    ...
    if(index.best.nbreak > 1) {
      ...
      if((testvec[index.best.nbreak]+2) > testvec[index.best.nbreak-1]) {
